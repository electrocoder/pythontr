...
(function() {
	function serialize(o, quote) {
		...
		if (t == "object") {
			...
			v = "{";
			for (i in o) {
				v += typeof o[i] != "function" ? (v.length > 1 ? "," + quote
						: quote)
...
(function(l) {
	...
	function a(q, p) {
		...
		m(p).replace(/#/g, "#text").replace(/(\w+)\[([^\]]+)\]\[([^\]]*)\]/g,
				function(u, s, r, t) {
					r = h(r, "|");
					o[s] = {
						attributes : c(r),
	...
	l.html.Schema = function(p) {
		...
		if (!p.valid_elements) {
			...
			j(h("ol,ul,li,sub,sup,blockquote,tr,div,span,font,a,table,tbody"),
					function(y) {
						m[y].removeEmpty = true
					});
			j(h("p,h1,h2,h3,h4,h5,h6,th,td,pre,div,address,caption"), function(
					y) {
				m[y].paddEmpty = true
			})
		} else {
			...
			.create(
					"tinymce.dom.DOMUtils",
					{
						...
						DOMUtils : function(n, l) {
							...
							if (h.isIE6) {
								try {
									n.execCommand("BackgroundImageCache",
											false, true)
								} catch (m) {
									k.cssFlicker = true
								}
						...
						setAttrib : function(l, m, j) {
							...
							return this.run(l, function(o) {
								...
								case "href":
									if (n.keep_values) {
										if (n.url_converter) {
											j = n.url_converter.call(
													n.url_converter_scope || k,
													j, m, o)
										}
						...
						getPos : function(s, m) {
							...
							if (s) {
								if (b && !k.stdMode) {
									...
									j = k.getStyle(k.select("html")[0],
											"borderWidth");
									j = (j == "medium" || k.boxModel
											&& !k.isIE6)
											&& 2 || j;
									return {
							...
							function k(v) {
								...
								if (u != 9) {
									if (u == 3 && v.nodeValue.length > 0) {
										if (!s.isBlock(v.parentNode)
												|| h.trim(v.nodeValue).length > 0) {
											return
										}
									} else {
										if (u == 1) {
			...
			.create(
					"tinymce.ui.ListBox:tinymce.ui.Control",
					{
						...
						selectByIndex : function(f) {
							...
							if (f != g.selectedIndex) {
								...
								if (i) {
									...
									c.setAttrib(g.id, "aria-valuenow",
											g.settings.title)
								}
								h = 0
							}
						...
						renderHTML : function() {
							...
							i = '<span role="button" aria-haspopup="true" aria-labelledby="'
									+ f.id
									+ '_text" aria-describedby="'
									+ f.id
	...
	m
			.create(
					"tinymce.Editor",
					{
						...
						init : function() {
							var r, F = this, G = F.settings, C, z, B = F
									.getElement(), q, p, D, x, A, E, y;
							m.add(F);
							G.aria_label = G.aria_label
									|| n.getAttrib(B, "aria-label", F
							...
							i(g(G.plugins.replace(/\-/g, "")),
									function(H) {
										var I = c.get(H), t = c.urls[H]
												|| m.documentBaseURL.replace(
														/\/$/, ""), s;
										if (I) {
											s = new I(F, t);
							...
							if (G.custom_undo_redo) {
								F.onBeforeExecCommand.add(function(t, H, u, I,
										s) {
									if (H != "Undo" && H != "Redo"
											&& H != "mceRepaint"
											&& (!s || !s.skip_undo)) {
							...
							if (G.content_css) {
								m.each(g(G.content_css), function(s) {
									F.contentCSS.push(F.documentBaseURI
											.toAbsolute(s))
								})
							}
							z = (q.iframeHeight || z)
									+ (typeof (z) == "number" ? (q.deltaHeight || 0)
											: "");
							if (z < 100) {
								z = 100
							}
							...
							if (!a || !/Firefox\/2/.test(navigator.userAgent)) {
								for (y = 0; y < F.contentCSS.length; y++) {
									F.iframeHTML += '<link type="text/css" rel="stylesheet" href="'
											+ F.contentCSS[y] + '" />'
								}
								F.contentCSS = []
							}
						...
						setupIframe : function() {
							var r = this, x = r.settings, y = n.get(r.id), z = r
									.getDoc(), v, p;
							...
							r.serializer = new m.dom.Serializer(x, r.dom,
									r.schema);
							r.selection = new m.dom.Selection(r.dom,
						...
						nodeChanged : function(r) {
							var p = this, q = p.selection, u = q.getStart()
									|| p.getBody();
							if (p.initialized) {
								r = r || {};
						...
						setContent : function(t, s) {
							...
							if (s.format !== "raw") {
								t = new m.html.Serializer({}, r.schema)
										.serialize(r.parser.parse(t))
							}
							s.content = m.trim(t);
						...
						_addEvents : function() {
							...
							i(x,
									function(t, s) {
										...
										case "reset":
											...
											break;
										default:
											q.bind(C.content_editable ? B
													.getBody() : B.getDoc(), s,
													p)
										}
									});
							q.bind(C.content_editable ? B.getBody() : (a ? B
									.getDoc() : B.getWin()), "focus", function(
									s) {
								B.focus(true)
							});
							if (m.isGecko) {
							...
							if (a) {
								function u() {
									...
									if (a && !F.readonly) {
										...
										try {
											G.execCommand("styleWithCSS", 0,
													false)
										} catch (D) {
											if (!E._isHidden()) {
												try {
													G.execCommand("useCSS", 0,
															true)
												} catch (D) {
												}
											}
								...
								B.onClick.add(function(s, t) {
									...
									if (t.nodeName == "IMG"
											|| (t.nodeName == "A" && q
													.hasClass(t,
															"mceItemAnchor"))) {
										B.selection.getSel().setBaseAndExtent(
												t, 0, t, 1);
										B.nodeChanged()
									}
								})
							...
							if (C.custom_shortcuts) {
								if (C.custom_undo_redo_keyboard_shortcuts) {
									B.addShortcut("ctrl+z", B
											.getLang("undo_desc"), "Undo");
									B.addShortcut("ctrl+y", B
											.getLang("redo_desc"), "Redo")
								}
								B.addShortcut("ctrl+b", B.getLang("bold_desc"),
										"Bold");
								B.addShortcut("ctrl+i", B
										.getLang("italic_desc"), "Italic");
								B
										.addShortcut("ctrl+u", B
												.getLang("underline_desc"),
												"Underline");
								for (r = 1; r <= 6; r++) {
									B.addShortcut("ctrl+" + r, "", [
											"FormatBlock", false, "h" + r ])
								}
								B.addShortcut("ctrl+7", "", [ "FormatBlock",
										false, "<p>" ]);
								B.addShortcut("ctrl+8", "", [ "FormatBlock",
										false, "<div>" ]);
								B.addShortcut("ctrl+9", "", [ "FormatBlock",
										false, "<address>" ]);
								function v(t) {
									var s = null;
							...
							if (C.custom_undo_redo) {
								...
								B.dom.bind(B.dom.getRoot(), "dragend",
										function(s) {
											y()
										});
								B.onKeyUp
										.add(function(t, F) {
											...
											if ((F.keyCode >= 33 && F.keyCode <= 36)
													|| (F.keyCode >= 37 && F.keyCode <= 40)
													|| F.keyCode == 13
													|| F.keyCode == 45
													|| F.ctrlKey) {
												y()
							...
							if (m.isGecko) {
								function A() {
									var s = B.dom.getAttribs(B.selection
								...
								B.onKeyPress.add(function(s, D) {
									...
									if ((D.keyCode == 8 || D.keyCode == 46)
											&& z()) {
										t = A();
										B.getDoc().execCommand("delete", false,
												null);
										t();
										return j.cancel(D)
									}
	...
	c.EditorCommands = function(n) {
		...
		t({
			...
			mceInsertContent : function(z, D, E) {
				...
				function v(I, J, H) {
					var K = new c.dom.TreeWalker(H ? I.nextSibling
							: I.previousSibling, J);
					while ((I = K.current())) {
						if ((I.nodeType == 3 && c.trim(I.nodeValue).length)
								|| I.nodeName == "BR" || I.nodeName == "IMG") {
							return I
						}
						if (H) {
							K.next()
				...
				l
						.setOuterHTML(
								...
								E
										.replace(/\{\$caret\}/,
												'<span data-mce-type="bookmark" id="__mce">\uFEFF</span>'));
				C = l.select("#__mce")[0];
				x = l.getRoot();
				if (C.previousSibling && l.isBlock(C.previousSibling)
						|| C.parentNode == x) {
					y = v(C, x);
					if (y) {
						if (y.nodeName == "BR") {
				...
				while (C) {
					if (C === x) {
						l.setOuterHTML(F, new c.html.Serializer({}, n.schema)
								.serialize(n.parser.parse(l.getOuterHTML(F))));
						break
					}
					F = C;
					C = C.parentNode
			...
			mceReplaceContent : function(x, v, u) {
				n.execCommand("mceInsertContent", false, u.replace(
					...
					"tinymce.ForceBlocks",
					{
						...
						find : function(u, p, q) {
							var o = this.editor, m = o.getDoc()
									.createTreeWalker(u, 4, null, g), r = -1;
							while (u = m.nextNode()) {
								r++;
								if (p == 0 && u == q) {
									return r
...
(function(a) {
	a.Formatter = function(V) {
		var M = {}, O = a.each, c = V.dom, q = V.selection, t = a.dom.TreeWalker, K = new a.dom.RangeUtils(
				c), d = V.schema.isValidChild, F = c.isBlock, l = V.settings.forced_root_block, s = c.nodeIndex, E = "\uFEFF", e = /^(src|href|style)$/, S = false, B = true, p, P = {
			apply : [],
			remove : []
		};
		function z(W) {
				...
				K
						.walk(
								...
								function(an) {
									var ao;
									function ap(aq) {
										var au = aq.nodeName.toLowerCase(), at = aq.parentNode.nodeName
												.toLowerCase(), ar;
										if (g(au, "br")) {
											ao = 0;
											if (ag.block) {
										...
										if (d(am, au)
												&& d(at, am)
												&& !(aq.nodeType === 3
														&& aq.nodeValue.length === 1 && aq.nodeValue
														.charCodeAt(0) === 65279)) {
											if (!ao) {
												ao = al.cloneNode(S);
												aq.parentNode.insertBefore(ao,
														aq);
												aj.push(ao)
											}
											ao.appendChild(aq)
			...
			if (ag) {
				if (aa) {
					...
					if (!ac || !ag.inline
						...
						ah(o(q.getRng(B), ab));
						if (ag.styles
								&& (ag.styles.color || ag.styles.textDecoration)) {
		...
		function A(Y, ah, ab) {
			...
			function ae(an) {
				var am = c.get(an ? "_start" : "_end"), al = am[an ? "firstChild"
						: "lastChild"];
				if (H(al)) {
		...
		function o(W, ag, Z) {
			...
			function ab(ak, al) {
				...
				while (ak && ak.hasChildNodes()) {
					ak = ak.childNodes[al];
					if (ak) {
						al = ak.nodeType === 3 ? ak.length
								: ak.childNodes.length
					}
				}
				return {
					node : ak,
			...
			if (ag[0].block || ag[0].selector) {
				function X(al, ak, an) {
					...
					if (!am) {
						am = c.getParent(al.nodeType == 3 ? al.parentNode : al,
								F)
					}
					if (am && ag[0].wrapper) {
